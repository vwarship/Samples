...
<LinearLayout xmlns:android="http://schemas.android.com/apk/res/android"
    ...
    <LinearLayout
        ...
        <Button
            ...
            android:layout_height="wrap_content"
            android:id="@+id/btnShowNewsContent"
            android:text="显示新闻内容"
            android:onClick="onShowNewsContentClick"/>
        <Button
            android:layout_width="wrap_content"
            android:layout_height="wrap_content"
            android:id="@+id/btnRemoveNewsContent"
            android:text="移除新闻内容"
            android:onClick="onRemoveNewsContentClick"/>
    </LinearLayout>

    <FrameLayout
        android:id="@+id/news_title_list_container"
        android:layout_width="match_parent"
...
```
* visibility="gone" 不占用空间，这样 news_title_list_container 就可以占用整个布局。

#### fragment_news_title_list.xml
```xml
...
```
* Fragment 使用动画的两种方法。
  * 1. fragmentTransaction.setTransition(FragmentTransaction.TRANSIT_FRAGMENT_OPEN);
  * 2. fragmentTransaction.setCustomAnimations(R.anim.abc_slide_in_bottom, R.anim.abc_slide_out_top);

#### NewsTitleListFragment.java
```java
...
```
* 定义 OnNewsSelectedListener 事件接口，用于与 Activity 交互。
